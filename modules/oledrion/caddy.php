...
function listCart()
{
    ...
    $reductions_return = $reductions->computeCart($cartForTemplate, $emptyCart, $shippingAmount, $commandAmount, $vatAmount, $goOn, $commandAmountTTC, $discountsDescription, $discountsCount, $ecotaxeAmount, $discountAmount, $totalSavings);
    $oledrion_Currency = oledrion_Currency::getInstance();
    $xoopsTpl->assign('emptyCart', $reductions_return['emptyCart']); // Caddy Vide ?
    $xoopsTpl->assign('caddieProducts', $reductions_return['cartForTemplate']); // Produits dans le caddy
    $xoopsTpl->assign('shippingAmount', $oledrion_Currency->amountForDisplay($reductions_return['shippingAmount'])); // Montant des frais de port
    $xoopsTpl->assign('ecotaxeAmount', $oledrion_Currency->amountForDisplay($reductions_return['ecotaxeAmount'])); // Montant des frais de port
    $xoopsTpl->assign('commandAmount', $oledrion_Currency->amountForDisplay($reductions_return['commandAmount'])); // Montant HT de la commande
    $xoopsTpl->assign('discountAmount', $oledrion_Currency->amountForDisplay($reductions_return['discountAmount'])); // Total Discount
    $xoopsTpl->assign('totalSavings', $oledrion_Currency->amountForDisplay($reductions_return['totalSavings'])); // Total Savings
    $xoopsTpl->assign('vatAmount', $oledrion_Currency->amountForDisplay($reductions_return['vatAmount'])); // Montant de la TVA
    $xoopsTpl->assign('discountsCount', $reductions_return['discountsCount']); // Nombre de réductions appliquées
    $xoopsTpl->assign('goOn', $reductions_return['goOn']); // Adresse à utiliser pour continuer ses achats
    $xoopsTpl->assign('commandAmountTTC', $oledrion_Currency->amountForDisplay($reductions_return['commandAmountTTC'], 'l')); // Montant TTC de la commande
    $xoopsTpl->assign('discountsDescription', $reductions_return['discountsDescription']); // Liste des réductions accordées
