...
$content ['imageurl'] = XOOPS_URL . '/uploads/news/image/medium/' . $content ['story_img'];
$content ['thumburl'] = XOOPS_URL . '/uploads/news/image/thumb/' . $content ['story_img'];
...
if (isset ( $story_topic ) && $story_topic > 0 && $view_topic->getVar ( 'topic_showtype' ) != '0') { // The option for select setting from topic or module options must be added
	...
	if ($view_topic->getVar ( 'topic_showpdf' )) {
		$link ['pdf'] = NewsUtils::News_Url ( $content, 'pdf' );
	}
	if ($view_topic->getVar ( 'topic_showprint' )) {
		$link ['print'] = NewsUtils::News_Url ( $content, 'print' );
	}
	...
	if ($view_topic->getVar ( 'topic_showmail' )) {
		...
		$link ['mail_linkto'] = NewsUtils::News_Url ( $content );
...
// Get URLs 
$link ['url'] = NewsUtils::News_Url ( $content );
$link ['topicurl'] = NewsUtils::News_TopicUrl ( $content );

// breadcrumb
if (xoops_getModuleOption ( 'bc_show', 'news' )) {
	$breadcrumb = NewsUtils::News_Breadcrumb ( true, $content ['story_title'], $content ['story_topic'], ' &raquo; ', 'topic_title' );
}
